...
class ProfileController extends GetxController {
  ...
  Future<void> getUserInfo() async {
    ...
    try {
      var response = await dio.get("https://homebrigadier.fly.dev/api/iam/");
      if (response.statusCode == 200) {
        ...
        services.clear();
        services.value = servicesList.map((service) => Service.fromJson(service)).toList();
...
class SettingList {
  const SettingList({required this.leading, required this.title, this.trailing});
