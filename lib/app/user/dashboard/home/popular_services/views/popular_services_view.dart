...
class TabBarGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final width = MediaQuery.of(context).size.width; // Correct typo in variable name
    final height = MediaQuery.of(context).size.height;
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(child: CircularProgressIndicator()); // Remove const keyword
        } else if (snapshot.hasError) {
          return Text('Error: ${snapshot.error}');
        } else {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              double endLatitude = emp.isNotEmpty ? emp[0] : 0; // Add null check
              double endLongitude = emp.isNotEmpty ? emp[1] : 0; // Add null check
